...
TBOOL TPrimShader::AddMesh()
{
	...
	TRenderDX11::Interface()->m_pDeviceContext->Map(m_pBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &subResource);
...
void TPrimShader::DrawPrim(PrimType primType, UINT numPrim, UINT startPrim)
{
	...
	switch (primType)
	{
		case 0: primitive = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP; break;
		case 1: primitive = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST; break;
		case 2:
		...
		case 3:
			...
			TASSERT((numPrim & 1) == 0);
			break;
		default: TASSERT(TFALSE, "Unknown prim type");
	}
...
void TPrimShader::Initialize()
{
	...
	// Vertex shader
	pBlob = pRender->CompileShaderFromFile("Data/Shaders/Other/PrimShader.vs", "main", "vs_4_0_level_9_3", TNULL);
	...
	pBlob = pRender->CompileShaderFromFile("Data/Shaders/Other/PrimShader.ps", "main", "ps_4_0_level_9_3", TNULL);
	pRender->CreatePixelShader(pBlob->GetBufferPointer(), pBlob->GetBufferSize(), &m_pPShaderImage);
	pBlob->Release();

	// Pixel shader for color rendering
	pBlob = pRender->CompileShaderFromFile("Data/Shaders/Other/PrimShaderNoTex.ps", "main", "ps_4_0_level_9_3", TNULL);
	...
	m_pBuffer = pRender->CreateBuffer(0, ONE_VERTEX_BUFFER_SIZE * sizeof(TPrimShader::Vertex), TNULL, D3D11_USAGE_DYNAMIC, D3D11_CPU_ACCESS_WRITE);
}
