...
void TInputDXDeviceMouse::Update(TFLOAT deltaTime)
{
	if (IsAcquired())
	{
		...
		if (renderer != TNULL)
		{
			...
			if (params->Height == 0)
			{
				TINT x = TINT(m_CursorPos.x);
				TINT y = TINT(m_CursorPos.y);
				TMath::Clip(x, 0, TINT(params->Width));
				TMath::Clip(y, 0, TINT(params->Height));
			}
...
TINT TInputDXDeviceMouse::ProcessEvents(EventEmitter& emitter, TFLOAT deltaTime)
{
	...
	for (size_t i = 0; i < dwItems; i++)
	{
		switch (dod[i].dwOfs)
		{
			...
			case DIMOFS_BUTTON2:
			{
				TINT iButton = (dod[i].dwOfs == DIMOFS_BUTTON0) ? (!m_bReverseButtons ? 0 : 1) : (dod[i].dwOfs == DIMOFS_BUTTON1) ? (!m_bReverseButtons ? 1 : 0) :
				                                                                                                                    2;
...
TBOOL const TInputDXDeviceMouse::BindToDIDevice(HWND a_mainWindow, LPCDIDEVICEINSTANCEA a_poDeviceInstance, IDirectInputDevice8A* a_poDXInputDevice, TBOOL exclusive)
{
	...
	if (hr != DI_OK) return TFALSE;

	if (a_mainWindow) m_poDXInputDevice->SetCooperativeLevel(a_mainWindow, DISCL_FOREGROUND | (exclusive ? DISCL_EXCLUSIVE : DISCL_NONEXCLUSIVE));
...
TINT TInputDXDeviceMouse::GetAxisInt(TINT doodad, TINT coord) const
{
	...
	{
		if (coord == 0) return TINT(m_aAxis.m_fX);

		if (coord == 1) return TINT(m_aAxis.m_fY);
	}
	else if (doodad == AXIS_WHEEL)
		return TINT(m_fWheelAxis);
...
TFLOAT TInputDXDeviceMouse::GetAxisFloat(TINT doodad, TINT coord) const
{
	if (doodad == AXIS_CURSOR)
	{
		if (coord == 0) return m_aAxis.m_fX;

		if (coord == 1) return m_aAxis.m_fY;
	}
	else if (doodad == AXIS_WHEEL)
		return m_fWheelAxis;
