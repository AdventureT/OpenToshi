...
TSysShaderHAL::TSysShaderHAL(OrderTablePriorities* priorities)
{
	...
	{
		m_Priorities = { 100, 200, 300, 400, 500, 500, 400 };
	}
}
...
TBOOL TSysShaderHAL::Create()
{
	...
	auto pShader = pRender->CompileShaderFromFile("Data/Shaders/SysShader/SysShader.vs", "main", "vs_4_0_level_9_3", TNULL);
	pRender->CreateVertexShader(pShader->GetBufferPointer(), pShader->GetBufferSize(), &m_pVertexShader);
	...
	pDevice->CreateInputLayout(inputDescs, TARRAYSIZE(inputDescs), pShader->GetBufferPointer(), pShader->GetBufferSize(), &m_pInputLayout);
	pShader->Release();

	// Create main pixel shader
	pShader = pRender->CompileShaderFromFile("Data/Shaders/SysShader/SysShader.ps", "main", "ps_4_0_level_9_3", TNULL);
	pRender->CreatePixelShader(pShader->GetBufferPointer(), pShader->GetBufferSize(), &m_pPixelShaderNoTex);
	pShader->Release();

	// Create pixel shader with support of a texture
	pShader = pRender->CompileShaderFromFile("Data/Shaders/SysShader/SysShader_Texture.ps", "main", "ps_4_0_level_9_3", TNULL);
	pRender->CreatePixelShader(pShader->GetBufferPointer(), pShader->GetBufferSize(), &m_pPixelShaderTexture);
	pShader->Release();

	// Create pixel shader with support of an alpha texture
	pShader = pRender->CompileShaderFromFile("Data/Shaders/SysShader/SysShader_AlphaTexture.ps", "main", "ps_4_0_level_9_3", TNULL);
	pRender->CreatePixelShader(pShader->GetBufferPointer(), pShader->GetBufferSize(), &m_pPixelShaderAlphaTexture);
	pShader->Release();
