...
{
private:
	inline static auto MallocNative = [](size_t size) -> void* {
		return malloc(size);
	};
	inline static auto CallocNative = [](size_t nitems, size_t size) -> void* {
		return calloc(nitems, size);
	};
	inline static auto ReallocNative = [](void* ptr, size_t size) -> void* {
		return realloc(ptr, size);
	};
	inline static auto MemalignNative = [](size_t alignment, size_t size) -> void* {
		return malloc(size);
	};
	inline static auto FreeNative = [](void* ptr) -> void {
		free(ptr);
	};
