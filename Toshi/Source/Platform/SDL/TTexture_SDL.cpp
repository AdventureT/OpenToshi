...
void TTexture::Init()
{
	...
	if (pFile != TNULL)
	{
		...
		TUINT8* mem      = (TUINT8*)TMemalign(16, fileSize + 1);
		pFile->Read(mem, fileSize);
		pFile->Destroy();
		mem[fileSize] = '\0';

		TINT    width, height, channels;
		TUINT8* texData = SOIL_load_image_from_memory(mem, fileSize, &width, &height, &channels, 4);
		TFree(mem);
	...
	{
		TINT    width, height, channels;
		TUINT8* texData = SOIL_load_image_from_memory(m_TexData, m_DataSize, &width, &height, &channels, 4);
...
void TTexture::Deinit()
{
	...
	if (m_pPrevTexture) m_pPrevTexture->m_pNextTexture = m_pNextTexture;

	if (m_pNextTexture) m_pNextTexture->m_pPrevTexture = m_pPrevTexture;

	auto pTexManager = TTextureManager::GetSingleton();

	if (pTexManager->GetLastTexture() == this) pTexManager->SetLastTexture(m_pPrevTexture);
...
TTexture* TTexture::InitRunTime(TEXTURE_FORMAT format, UINT width, UINT height, const void* srcData)
{
	...
	GLenum glFormat = (format == TEXTURE_FORMAT_R8G8B8A8_UNORM) ? GL_RGBA : (format == TEXTURE_FORMAT_R8G8_UNORM) ? GL_RG :
	                                                                                                                GL_R;
    ...
    : m_pLastTexture(TNULL)
{
	...
	// White texture
	constexpr TINT WhiteTextureWidth  = 8;
	constexpr TINT WhiteTextureHeight = 8;
	constexpr TINT WhiteTextureSize   = WhiteTextureWidth * WhiteTextureHeight * 4;

	srcData = (TUINT8*)TMemalign(16, WhiteTextureSize);
	...
	// Invalid texture
	constexpr TINT InvalidTextureWidth  = 128;
	constexpr TINT InvalidTextureHeight = 128;
	constexpr TINT InvalidTextureSize   = InvalidTextureWidth * InvalidTextureHeight * 4;

	srcData = (TUINT8*)TMemalign(16, InvalidTextureSize);

	for (TUINT8* pos = srcData; pos < srcData; pos += 4)
	{
		*(pos)     = rand() % 256;
		*(pos + 1) = rand() % 256;
		*(pos + 2) = rand() % 256;
