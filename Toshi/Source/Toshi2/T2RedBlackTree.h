...
public:
	...
	template <class T> friend class T2RedBlackTree;
...
protected:
	...
	size_t GetNumElements() const { return m_iNumElements; }
...
template <class T> class T2RedBlackTreeNode : public T2GenericRedBlackTreeNode
{
public:
	template <class T> friend class T2RedBlackTree;
...
public:
	...
	TBOOL IsLeftNodeNext(const T& value) const
	{
		...
		if (TComparator<T>::IsEqual(m_Value, value)) return TFALSE;

		return TComparator<T>::IsLess(m_Value, value);
	}

	TBOOL operator==(const T& other) const { return TComparator<T>::IsEqual(m_Value, other); }

	TBOOL operator==(const T2RedBlackTreeNode<T>& other) const { return TComparator<T>::IsEqual(m_Value, other.m_Value); }

	TBOOL operator!=(const T& other) const { return !TComparator<T>::IsEqual(m_Value, other); }

	TBOOL operator!=(const T2RedBlackTreeNode<T>& other) const { return !TComparator<T>::IsEqual(m_Value, other.m_Value); }

	TBOOL operator>(const T2RedBlackTreeNode<T>& other) const { return TComparator<T>::IsGreater(m_Value, other.m_Value); }

	TBOOL operator<(const T2RedBlackTreeNode<T>& other) const { return TComparator<T>::IsLess(m_Value, other.m_Value); }

	TBOOL operator<=(const T2RedBlackTreeNode<T>& other) const { return TComparator<T>::IsLessOrEqual(m_Value, other.m_Value); }

	TBOOL operator>=(const T2RedBlackTreeNode<T>& other) const { return TComparator<T>::IsGreaterOrEqual(m_Value, other.m_Value); }

private:
	T m_Value;
};

template <class T> class T2RedBlackTree : public T2GenericRedBlackTree
{
public:
	...
	public:
		...
		TFORCEINLINE Node* GetNode() const { return m_pNode; }

		TFORCEINLINE T* GetValue() { return &m_pNode->m_Value; }

		TFORCEINLINE TBOOL operator==(const Iterator& other) const { return m_pNode == other.m_pNode; }

		TFORCEINLINE T& operator*() const { return m_pNode->m_Value; }

		TFORCEINLINE T* operator->() const { return &m_pNode->m_Value; }

		TFORCEINLINE operator T*() const { return &m_pNode->m_Value; }

		TFORCEINLINE Iterator operator++(TINT) { return Next(); }
		...
		TFORCEINLINE Iterator operator--(TINT) { return Prev(); }
	...
	public:
		...
		TFORCEINLINE const Node* GetNode() const { return m_pNode; }

		TFORCEINLINE const T* GetValue() const { return &m_pNode->m_Value; }

		TFORCEINLINE TBOOL operator==(const Iterator& other) const { return m_pNode == other.GetNode(); }

		TFORCEINLINE TBOOL operator==(const CIterator& other) const { return m_pNode == other.m_pNode; }

		TFORCEINLINE const T& operator*() const { return m_pNode->m_Value; }

		TFORCEINLINE const T* operator->() const { return &m_pNode->m_Value; }

		TFORCEINLINE operator const T*() const { return &m_pNode->m_Value; }

		TFORCEINLINE CIterator operator++(TINT) { return Next(); }
		...
		TFORCEINLINE CIterator operator--(TINT) { return Prev(); }
...
public:
	...
	TBOOL IsRoot(Node* pNode) { return pNode == &m_oRoot; }

	Iterator Begin() { return Iterator(TREINTERPRETCAST(Node*, GetFirstNode())); }

	Iterator End() { return Iterator(TREINTERPRETCAST(Node*, &m_oRoot)); }

	const CIterator Begin() const { return CIterator(TREINTERPRETCAST(const Node*, GetFirstNode())); }

	const CIterator End() const { return CIterator(TREINTERPRETCAST(const Node*, &m_oRoot)); }
	...
	void Delete(Node* a_pNode) { m_pAllocator->Delete(DeleteNode(a_pNode)); }
	...
	TSIZE Size() const { return GetNumElements(); }
