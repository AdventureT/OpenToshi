...
template <class T, TINT Unknown = 0> class T2SList
{
public:
	class Node
	{
	...
	public:
		...
		T* GetNext() const { return TSTATICCAST(T, m_pNext); }
...
public:
	...
	T* Begin() { return m_oRoot.GetNext(); }

	T* End() { return TREINTERPRETCAST(T*, &m_oRoot); }

	TBOOL IsLinked() const { return m_oRoot.m_pNext != &m_oRoot; }
	...
	void PushBack(T* a_pNode) { a_pNode->InsertAfter(Back()); }
