...
template <typename T, size_t MaxSize> class T2Vector
{
public:
	class Iterator
	{
	...
	public:
		Iterator(T2Vector* a_pVector)
		    : m_uiIndex(0), m_pVector(a_pVector) {}
		Iterator(size_t a_uiIndex, T2Vector* a_pVector)
		    : m_uiIndex(a_uiIndex), m_pVector(a_pVector) {}
		Iterator(const Iterator& a_rOther)
		    : m_uiIndex(a_rOther.m_uiIndex), m_pVector(a_rOther.m_pVector) {}

		size_t Index() const noexcept { return m_uiIndex; }

		T* Pointer() noexcept { return &m_pVector->At(m_uiIndex); }

		const T* Pointer() const noexcept { return &m_pVector->At(m_uiIndex); }

		T& Value() noexcept { return m_pVector->At(m_uiIndex); }

		const T& Value() const noexcept { return m_pVector->At(m_uiIndex); }

		T& operator*() noexcept { return m_pVector->ValueAt(m_uiIndex); }

		const T& operator*() const noexcept { return m_pVector->ValueAt(m_uiIndex); }

		T* operator->() noexcept { return &m_pVector->ValueAt(m_uiIndex); }

		const T* operator->() const noexcept { return &m_pVector->ValueAt(m_uiIndex); }

		TBOOL operator==(const Iterator& a_rOther) const noexcept { return a_rOther.m_pVector == m_pVector && a_rOther.m_uiIndex == m_uiIndex; }
...
public:
	T2Vector() { m_iNumElements = 0; }

	T* Push() { return &m_Items[m_iNumElements++]; }
	...
	void Clear()
	{
		for (size_t i = 0; i < Size(); i++) m_Items[i].~T();
	...
	Iterator Find(const T& a_rValue)
	{
		...
		{
			if (it.GetValue() == a_rValue) return it;
		}
	}
	...
	void Erase(const Iterator& a_rIterator)
	{
		...
		{
			for (size_t i = uiItemIndex + 1; i < Size(); i++) m_Items[i - 1] = m_Items[i];
		}
		else
		{
	...
	size_t Size() const { return m_iNumElements; }

	TBOOL IsEmpty() const { return Size() == 0; }
