...
protected:
	virtual void* Malloc(size_t a_uiSize, size_t a_uiAlignment) override { return TNULL; }

	virtual void* Malloc(size_t a_uiSize) override { return GetObject(); }

	virtual void Free(void* a_pPtr) override { ReturnObject(TSTATICCAST(Object, a_pPtr)); }

	virtual TBOOL CanAllocate(size_t size) override { return m_pHead != TNULL; }

	virtual void* TryMalloc(size_t size, size_t alignment) override { return TNULL; }
...
template <class T, TUINT MaxNumber, TUINT ObjectSize = sizeof(T), TUINT Alignment = alignof(T)> class T2ObjectPool : protected T2GenericObjectPool
{
public:
...
public:
	T2ObjectPool() { T2GenericObjectPool::Initialise(TREINTERPRETCAST(T2GenericObjectPool::Object*, GetObjects()), MaxNumber, ObjectSize); }

	template <class... Args> T* NewObject(Args&&... args)
	{
	...
	virtual size_t GetUsedSize() override { return (MaxNumber - GetNumFreeObjects()) * ObjectSize; }

	virtual size_t GetCapacity() override { return MaxNumber * ObjectSize; }

	virtual TBOOL IsAddressInPool(const void* a_pAddress)
	{
		return (TREINTERPRETCAST(uintptr_t, this) + sizeof(T2GenericObjectPool)) <= TREINTERPRETCAST(uintptr_t, a_pAddress) &&
		    TREINTERPRETCAST(uintptr_t, a_pAddress) < (TREINTERPRETCAST(uintptr_t, this) + sizeof(T2GenericObjectPool) + (MaxNumber * ObjectSize));
	}

private:
	T* GetObjects() { return TREINTERPRETCAST(T*, m_aObjects); }

	const T* GetObjects() const { return TREINTERPRETCAST(const T*, m_aObjects); }

private:
	TUINT8 m_aObjects[MaxNumber * ObjectSize];
};

template <class TClassType> class T2DerivedDynamicObjectPool : protected T2GenericObjectPool
{
public:
	T2DerivedDynamicObjectPool(T2Allocator* a_pAllocator, TINT a_iMaxNumber)
	{
		...
		TVALIDPTR(m_pData);
		T2GenericObjectPool::Initialise(TREINTERPRETCAST(T2GenericObjectPool::Object*, m_pData), m_iMaxNumber, m_uiObjectSize);
	}

	virtual ~T2DerivedDynamicObjectPool() { m_pAllocator->Free(m_pData); }

	template <class... Args> TClassType* NewObject(Args&&... args)
	{
	...
	template <class T, class... Args> T* NewDerivedObject(Args&&... args)
	{
	...
	virtual size_t GetUsedSize() override { return (m_iMaxNumber - GetNumFreeObjects()) * m_uiObjectSize; }

	virtual size_t GetCapacity() override { return m_iMaxNumber * m_uiObjectSize; }

	virtual TBOOL IsAddressInPool(const void* a_pAddress)
	{
		return TREINTERPRETCAST(uintptr_t, m_pData) <= TREINTERPRETCAST(uintptr_t, a_pAddress) && TREINTERPRETCAST(uintptr_t, a_pAddress) < TREINTERPRETCAST(uintptr_t, m_pData) + (m_iMaxNumber * m_uiObjectSize);
	}
