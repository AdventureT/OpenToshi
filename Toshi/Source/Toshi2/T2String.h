...
//-----------------------------------------------------------------------------
template <class StringTraits = T2StringTraits<TCHAR>> class T2StringViewBase
{
public:
...
public:
	...
	TFORCEINLINE constexpr T2StringViewBase operator+(TINT a_iSize) const { return T2StringViewBase(m_pszString + a_iSize); }
	TFORCEINLINE constexpr T2StringViewBase operator-(TINT a_iSize) const { return T2StringViewBase(m_pszString - a_iSize); }
	TFORCEINLINE constexpr T2StringViewBase operator+=(TINT a_iSize)
	{
		m_pszString += a_iSize;
		return *this;
	}
	...
	TFORCEINLINE TBOOL operator==(T2StringViewBase a_otherString) { return StringTraits::Compare(m_pszString, a_otherString.m_pszString) == 0; }
...
// Note: To support other encodings, pass your own StringTraits class.
//-----------------------------------------------------------------------------
template <TSIZE Size, typename TStringTraits = T2StringTraits<TCHAR>> class T2FormatString
{
public:
...
public:
	constexpr T2FormatString() { m_szBuffer[0] = CharTraits::NullChar; }

	T2FormatString(const CharType* a_szString) { Copy(a_szString); }

	void Copy(const CharType* a_szString) { StringTraits::CopySafe(m_szBuffer, a_szString, Size); }
	...
	TINT ParseLine(const CharType* a_szString, TINT a_uiSize = -1, TINT* a_pStringLength = TNULL, TBOOL a_bTrimStartSpaces = TTRUE, TBOOL a_bTrimEndSpaces = TTRUE)
	{
		...
		if (a_uiSize == -1) a_uiSize = INT_MAX;
	...
	void Append(const CharType* a_szString) { StringTraits::Concat(m_szBuffer, a_szString); }

	void Clear() { m_szBuffer[0] = CharTraits::NullChar; }

	TUINT Length() { return StringTraits::Length(m_szBuffer); }

	constexpr CharType* Get() { return m_szBuffer; }

	constexpr const CharType* Get() const { return m_szBuffer; }
	...
	constexpr CharType& operator[](TUINT a_uiIndex) { return *Get(a_uiIndex); }

	constexpr const CharType& operator[](TUINT a_uiIndex) const { return *Get(a_uiIndex); }
...
// Note: To support other encodings, pass your own StringTraits class.
//-----------------------------------------------------------------------------
template <class StringTraits = T2StringTraits<TCHAR>> class T2StringParser
{
public:
	using CharType    = typename StringTraits::CharType;
...
public:
	T2StringParser(ConstString a_pchBuffer, TSIZE a_uiBufferSize = -1) { SetBuffer(a_pchBuffer, a_uiBufferSize); }

	T2StringParser() { m_uiBufferSize = 0; }
	...
	void Reset() { m_BufferPos = m_Buffer; }

	TSIZE Tell() const { return m_BufferPos - m_Buffer; }

	void Set(TSIZE a_uiPos) { m_BufferPos = m_Buffer + a_uiPos; }
	...
	TBOOL PeekToken(CharType* a_pszToken, TSIZE a_uiTokenMaxSize)
	{
		...
		if (cursor >= m_BufferEnd && !bIsInQuotes) bIsValid = TTRUE;
	...
	TBOOL IsOver() const { return m_BufferPos >= m_BufferEnd; }
...
// Note: To support other encodings, pass your own StringTraits class.
//-----------------------------------------------------------------------------
template <class StringTraits = T2StringTraits<TCHAR>> class T2StringComparator
{
public:
	using CharType = typename StringTraits::CharType;

public:
	static TBOOL IsEqual(const CharType* a, const CharType* b) { return StringTraits::Compare(a, b) == 0; }

	static TBOOL IsGreater(const CharType* a, const CharType* b) { return StringTraits::Compare(a, b) > 0; }

	static TBOOL IsLess(const CharType* a, const CharType* b) { return StringTraits::Compare(a, b) < 0; }

	static TBOOL IsLessOrEqual(const CharType* a, const CharType* b) { return StringTraits::Compare(a, b) <= 0; }

	static TBOOL IsGreaterOrEqual(const CharType* a, const CharType* b) { return StringTraits::Compare(a, b) >= 0; }
};
...
template <TINT Size, typename TStringTraits = T2StringTraits<TCHAR>> class T2StringBuffer
{
public:
	TSTATICASSERT(Size > 0);
...
public:
	...
	T2StringBuffer(const CharType* a_szString) { Copy(a_szString); }
	...
	constexpr TINT Length() { return m_iPosition; }
