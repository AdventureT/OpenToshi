...
public:
	...
	__forceinline TFLOAT GetD() const { return m_fDistance; }

	void SetD(TFLOAT a_fDistance) { m_fDistance = a_fDistance; }

	__forceinline const TVector3& GetNormal() const { return m_Normal; }

	__forceinline void SetNormal(const TVector3& a_rNormal) { m_Normal = a_rNormal; }

	__forceinline void SetNormal(TFLOAT a_fNormalX, TFLOAT a_fNormalY, TFLOAT a_fNormalZ) { m_Normal.Set(a_fNormalX, a_fNormalY, a_fNormalZ); }

	__forceinline void GetPoint(TVector3& a_rPoint) { a_rPoint.Multiply(m_Normal, m_fDistance); }

	__forceinline void SetPoint(const TVector3& a_rPoint) { m_fDistance = TVector3::DotProduct(m_Normal, a_rPoint); }

	__forceinline void Negate(const TPlane& a_rPlane) { AsVector4().Negate4(a_rPlane.AsVector4()); }

	__forceinline void Negate() { AsVector4().Negate4(); }
