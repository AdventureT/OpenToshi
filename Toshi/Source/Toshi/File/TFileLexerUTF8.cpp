...
void TFileLexerUTF8::SetCharacterLookaheadSize(TINT a_iLookaheadSize)
{
	...
	for (TINT i = 0; i < iIntCount; i++) m_piCharLookahead[i] = m_pFile->GetCChar();
}
...
{
	TBOOL bAlreadyDefined = TFALSE;

	for (TINT i = 0; i < m_Defines.GetNumElements(); i++)
	{
