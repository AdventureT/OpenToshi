...
TTRB::ERROR TTSFI::PushForm()
{
	...
	if (m_CurrentHunk.Name != TFourCC("FORM") && m_CurrentHunk.Name != TFourCC("TSFL") && m_CurrentHunk.Name != TFourCC("TSFB"))
	{
...
void TTSFI::ReadCompressed(void* buffer, TUINT32 size)
{
	...
	if (error == TCOMPRESS_ERROR_OK)
	{
		...
		TCompress_Decompress::Decompress(m_pFile, &header, (TCHAR*)buffer, size);
