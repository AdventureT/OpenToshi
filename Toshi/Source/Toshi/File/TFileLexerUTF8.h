...
class TFileLexerUTF8 : public TGenericClassDerived<TFileLexerUTF8, TObject, "TFileLexerUTF8", TMAKEVERSION(1, 0), TFALSE>
{
public:
	struct Token
	{
	...
	{
	public:
		Token* GetTokens() { return TREINTERPRETCAST(Token*, this + 1); }

		static LookaheadTokens* Allocate(TINT a_iCount = 1) { return TREINTERPRETCAST(LookaheadTokens*, TMalloc(sizeof(LookaheadTokens) + sizeof(Token) * a_iCount)); }

		static void Free(Token* a_pHeadToken) { TFree(FromToken(a_pHeadToken)); }

		static LookaheadTokens* FromToken(Token* a_pHeadToken) { return TREINTERPRETCAST(LookaheadTokens*, TREINTERPRETCAST(uintptr_t, a_pHeadToken) - sizeof(LookaheadTokens)); }
