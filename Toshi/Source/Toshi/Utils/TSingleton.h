...
public:
	...
	template <typename... Args> __forceinline static T* CreateSingleton(Args&&... args) { return new T(std::forward<Args>(args)...); }

	// Creates Singleton instance for a derived class and returns it
	template <typename D, typename... Args> __forceinline static D* CreateSingleton(Args&&... args)
	{
	...
	__forceinline static TBOOL IsSingletonCreated() { return ms_pSingleton != TNULL; }

	// Returns pointer even if it's not allocated
	__forceinline static T* GetSingleton() { return ms_pSingleton; }
...
template <typename T> T* TSingleton<T>::ms_pSingleton = TNULL;
