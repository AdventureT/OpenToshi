...
void TPString8Pool::Get(TPooledString8*& a_pOutString, const TCHAR* a_szString, TBOOL* a_pWasInPool)
{
	a_pOutString = TNULL;
	...
	if (a_szString != TNULL)
	{
		...
		if (iStrLen != 0)
		{
			...
			{
				if (a_pWasInPool) *a_pWasInPool = TTRUE;
...
void TPString8Initialiser::Initialise(TPString8Pool* a_pStringPool)
{
	...
	for (TSIZE i = 0; i < m_iCount; i++)
	{
		...
		if (string.GetPooledString() && string.GetPooledString() != m_pStrings[i].m_pString8->GetPooledString())
		{
