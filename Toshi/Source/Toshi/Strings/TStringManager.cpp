...
TWCHAR* TStringManager::StringCharToUnicode(TWCHAR* a_UnicodeString, const TCHAR* a_CharString, TUINT32 a_iLength)
{
	...
	if (iCharStringLength < a_iLength || a_iLength == -1) a_iLength = iCharStringLength;

	for (size_t i = 0; i < a_iLength; i++) a_UnicodeString[i] = TWCHAR(a_CharString[i]);
...
TCHAR* TStringManager::StringUnicodeToChar(TCHAR* a_CharString, const TWCHAR* const& a_UnicodeString, TUINT32 a_iLength)
{
	...
	if (iUnicodeStringLength < a_iLength || a_iLength == -1) a_iLength = iUnicodeStringLength;

	for (size_t i = 0; i < a_iLength; i++) a_CharString[i] = TCHAR(a_UnicodeString[i]);
