...
public:
	TPooledString8(const TCHAR* a_szString, TPString8Pool* a_pPool, T2Allocator* a_pAllocator)
	    : m_oString(a_szString, a_pAllocator) { m_pPool = a_pPool; }
...
public:
	...
	void Get(TPooledString8*& a_pOutString, const TCHAR* a_szString, TBOOL* a_pWasInPool = TNULL);
	...
	void InitialiseStatic() { TIMPLEMENT(); }

	T2Allocator* GetAllocator() const { return m_pAllocator; }

	T2Map<const TCHAR*, TPooledString8*, T2String8Comparator>::Iterator Begin() { return m_oMap.Begin(); }

	T2Map<const TCHAR*, TPooledString8*, T2String8Comparator>::Iterator End() { return m_oMap.End(); }
...
public:
	...
	{
	public:
		static TBOOL IsEqual(const TPString8& a, const TPString8& b) { return a.GetPooledString() == b.GetPooledString(); }

		static TBOOL IsGreater(const TPString8& a, const TPString8& b) { return a.GetPooledString() > b.GetPooledString(); }

		static TBOOL IsLess(const TPString8& a, const TPString8& b) { return a.GetPooledString() < b.GetPooledString(); }

		static TBOOL IsLessOrEqual(const TPString8& a, const TPString8& b) { return a.GetPooledString() <= b.GetPooledString(); }

		static TBOOL IsGreaterOrEqual(const TPString8& a, const TPString8& b) { return a.GetPooledString() >= b.GetPooledString(); }
	};
...
public:
	...
	TFORCEINLINE explicit TPString8(const TCHAR* a_szString) { TSystemManager::GetStringPool()->Get(m_pPtr, a_szString, TNULL); }

	TFORCEINLINE TPString8(TPString8Pool* a_pPool, const TCHAR* a_szString) { a_pPool->Get(m_pPtr, a_szString, TNULL); }
	...
	TFORCEINLINE ~TPString8() { Decrement(); }

	TFORCEINLINE const TCHAR* GetString() const { return GetString8().GetString(); }

	TFORCEINLINE const TString8& GetString8() const { return m_pPtr ? m_pPtr->m_oString : ms_sEmpty; }

	TFORCEINLINE const TPooledString8* GetPooledString() const { return m_pPtr; }

	TFORCEINLINE TBOOL IsEmpty() const { return m_pPtr == TNULL || m_pPtr->GetString8().Length() == 0; }
	...
	TFORCEINLINE TBOOL IsEqual(const TPString8& a_Other) const { return m_pPtr == a_Other.m_pPtr; }

	TFORCEINLINE TBOOL IsEqual(const TPString8* a_pOther) const { return m_pPtr == a_pOther->m_pPtr; }

	TFORCEINLINE TBOOL operator==(const TPString8& a_Other) const { return m_pPtr == a_Other.m_pPtr; }
	...
	TFORCEINLINE operator const TCHAR*() const { return m_pPtr ? m_pPtr->m_oString.GetString() : ms_sEmpty.GetString(); }

	TFORCEINLINE operator const TString8*() const { return m_pPtr ? &m_pPtr->m_oString : &ms_sEmpty; }

	TFORCEINLINE operator const TPooledString8*() const { return m_pPtr; }
