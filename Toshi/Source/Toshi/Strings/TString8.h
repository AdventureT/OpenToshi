...
public:
	...
	TString8&       VFormat(const TCHAR* a_pcFormat, va_list a_pcArgs);
	...
	TINT32 Find(TCHAR character, TUINT32 pos = 0) const;

	// Returns position of specified substring
	TINT32 Find(const TCHAR* substr, TUINT32 pos = 0) const;
	...
	TString8& MakeUpper()
	{
		_strupr_s(m_pBuffer, Length());
		return *this;
	}
	TString8& MakeLower()
	{
		_strlwr_s(m_pBuffer, Length());
		return *this;
	}

	TUINT32 Length() const { return m_iStrLen; }
	TUINT8  ExcessLength() const { return m_iExcessLen; }
...
private:
	...
	T2Allocator* GetAllocator() { return sm_pDefaultAllocatorCB(); }

	static T2Allocator* GetDefaultAllocatorCB() { return GetGlobalAllocator(); }
