...
TINT32 TString8::Find(TCHAR character, TUINT32 pos) const
{
	...
	return (TINT32)(foundAt - m_pBuffer);
}

TINT32 TString8::Find(const TCHAR* substr, TUINT32 pos) const
{
	if (!IsIndexValid(pos)) return -1;
...
TString8& TString8::VFormat(const TCHAR* a_pcFormat, va_list a_pcArgs)
{
	TCHAR buffer[0x400];

	// TASSERT is only in T2String8
	TINT iResult = _vsnprintf(buffer, sizeof(buffer), a_pcFormat, a_pcArgs);
	TASSERT(iResult != -1, "PS2/GC/X360 do not correctly support _vsnprintf, this code will cause memory to be clobbered on those platforms! Increase the size of the destination string to avoid this problem")
	Copy(buffer, -1);
