...
namespace Toshi
{
	...
	TWString::TWString(uint32_t size)
	{
		Reset();
	...
	TWString& TWString::Concat(const wchar_t* str, uint32_t size)
	{
	...
	TBOOL TWString::AllocBuffer(uint32_t a_iLength, TBOOL freeMemory)
	{
	...
	TWString TWString::Mid(uint32_t param_1, uint32_t param_2) const
	{
		if (param_2 < 0)
		{
			param_2 = 0;
		...
		// Rewrite not correct
		TWString str = TWString(Length() - param_2);
	...
	void TWString::Copy(const TWString& src, uint32_t size)
	{
