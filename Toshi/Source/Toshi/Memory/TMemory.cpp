...
TMemoryHeap* TMemory::dlheapcreatesubheap(TMemoryHeap* heap, size_t size, TMemoryHeapFlags flags, const TCHAR name[HEAP_MAXNAME])
{
	...
	if (mem != TNULL)
	{
		if (flags & TMemoryHeapFlags_AllocAsPile)
		{
			...
			subHeap->m_PileData = reinterpret_cast<TCHAR*>(subHeap + 1);
		...
		if (subHeap != TNULL)
		{
			subHeap->m_SubHeapBuffer = static_cast<TCHAR*>(mem);
		}
		else
		{
			TFree(mem);
...
void* TMemoryHeap::AllocAsPile(TMemoryHeap* heap, size_t size, size_t alignment)
{
	...
	heap->m_PileData      = reinterpret_cast<TCHAR*>((((size_t)heap->m_PileData + (alignment - 1)) / alignment) * alignment);
