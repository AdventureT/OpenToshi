...
TBOOL TSound::Create(void* poolmem, TINT poollen, TINT maxchannels, TINT filebuffersize, SpeakerType speakerType)
{
	if (m_pSystem != NULL)
	{
...
TBOOL TSound::ErrorCheck(FMOD_RESULT error)
{
	if (error != FMOD_OK)
	{
		const TCHAR* errorStr = FMOD_ErrorString(error);
...
TBOOL TSound::InitMem(void* poolmem, TINT poollen)
{
	if (poolmem == NULL)
	{
		if (poollen < 0)
