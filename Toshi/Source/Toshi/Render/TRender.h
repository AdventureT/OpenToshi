...
public:
	...
	TFLOAT GetX() const { return m_oParams.fX; }

	TFLOAT GetY() const { return m_oParams.fY; }

	TFLOAT GetWidth() const { return m_oParams.fWidth; }

	TFLOAT GetHeight() const { return m_oParams.fHeight; }

	const TPlane* GetWorldPlanes();

	TBOOL CullSphereToFrustumSimple(const TSphere& a_rSphere, const TPlane* a_pPlanes, TINT a_iUnused);
...
public:
	...
	struct DisplayParams
	{
		...
		TBOOL   Unk5;
		TBOOL   IsFullscreen;
		TUINT32 MultisampleQualityLevel;
	};
...
public:
	...
	virtual void           Update(TFLOAT deltaTime);                            // 0x14 at vftable
