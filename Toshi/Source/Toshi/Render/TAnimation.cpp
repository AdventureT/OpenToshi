...
TBOOL TAnimation::UpdateTime(TFLOAT a_fDeltaTime)
{
	...
	if (pSeq->GetUnk2() == 0 && !HASFLAG(m_eFlags & Flags_Managed))
	{
		if (0.0f <= m_fSpeed || 0.0f <= m_fSeqTime)
		{
			if (fSeqDuration <= m_fSeqTime)
			{
				...
				for (TINT i = 0; i < pSkeleton->GetAutoBoneCount(); i++) *GetBone(i) = 0;
			}
		}
		else
		{
	...
	{
		if (m_fSeqTime < 0.0f && m_fSpeed < 0.0f) m_fSeqTime = fSeqDuration;
...
void TAnimation::RemoveAnimation(TFLOAT a_fBlendOutSpeed)
{
	if (m_pSkeletonInstance) m_pSkeletonInstance->RemoveAnimation(this, a_fBlendOutSpeed);
}
