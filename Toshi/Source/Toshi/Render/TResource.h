...
public:
	...
	TRender* GetRenderer() const { return m_Renderer; }

	void SetRenderer(TRender* pRenderer) { m_Renderer = pRenderer; }

	TResource* Parent() const { return TNode::Parent(); }

	TResource* GetNextResource() const { return TNode::Next(); }

	TResource* GetAttached() const { return TNode::Attached(); }

	TResource* GetLastResource() const { return TNode::Prev(); }

	TNodeTree<TResource>* GetTree() { return TNode::m_Tree; }

	TUINT32 GetUId() const { return m_UId; }

	void SetUId(TUINT32 uid) { m_UId = uid; }
