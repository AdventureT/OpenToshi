...
void TOrderTable::Flush()
{
	...
	if (m_fnPostFlushCallback) m_fnPostFlushCallback(m_pPostFlushCallbackData);
...
TRegMaterial* TOrderTable::RegisterMaterial(TMaterial* pMat)
{
	...
	if (pCurrentRegMat != TNULL) return pCurrentRegMat;

	TASSERT(TFALSE == s_llRegMatFreeList.IsEmpty());

	if (s_llRegMatFreeList.IsEmpty()) return TNULL;
...
void TRegMaterial::Render()
{
	...
	for (auto pPacket = m_pLastRenderPacket; pPacket != TNULL; pPacket = pPacket->Next()) pPacket->GetMesh()->GetOwnerShader()->Render(pPacket);
