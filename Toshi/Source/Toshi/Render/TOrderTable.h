...
public:
	...
	void SetNext(TRenderPacket* pNextPacket) { m_pNextPacket = pNextPacket; }

	void SetMaterial(TMaterial* pMaterial) { m_pMaterial = pMaterial; }

	TRenderPacket* Next() const { return m_pNextPacket; }

	TMesh* GetMesh() const { return m_pMesh; }

	TMaterial* GetMaterial() const { return m_pMaterial; }

	TMatrix44& GetModelViewMatrix() { return m_ModelViewMatrix; }
...
public:
	// HAL methods
	TBOOL Create(TShader* pShader, TINT priority, TUINT8 index);
...
public:
	...
	inline static TUINT32                 s_uiNumMaterials           = 0;
