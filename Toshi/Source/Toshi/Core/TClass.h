...
{
public:
	TClass(const TCHAR* a_szName, TClass* a_pParent, TUINT32 a_uiVersion, TUINT32 a_uiSize, t_CreateTObject a_fCreate, t_CreateTObjectInPlace a_fCreateInPlace, t_InitializeStatic a_fInit, t_UninitializeStatic a_fUninit, TUINT32 a_uiAlignment);

	constexpr TClass(const TCHAR* a_szName, TUINT32 a_uiVersion, t_CreateTObject a_fCreate, t_CreateTObjectInPlace a_fCreateInPlace, TUINT32 a_uiSize, TUINT32 a_uiAlignment)
	    : m_Name(a_szName)
	    , m_Create(a_fCreate)
	...
	static const TClass* Find(const TCHAR* name, const TClass* parent);

public:
	inline TBOOL operator==(const TClass* other) const { return this == other; }

protected:
	const TCHAR*           m_Name          = 0; // 0x00
