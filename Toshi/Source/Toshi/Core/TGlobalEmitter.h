...
template <class T> class TGenericGlobalListener : public TDList<TGenericGlobalListener<T>>::TNode
{
public:
	using EventCallback = void (*)(void*, const T&);

	template <class T> friend class TGlobalEmitter;
...
protected:
	...
	void Execute(const T& a_rData) { m_fnCallback(m_pReceiver, a_rData); }
...
template <class T> class TGlobalEmitter
{
public:
...
template <class ReceiverType, class EventType> class TGlobalListener : public TGenericGlobalListener<EventType>
{
public:
	void Connect(ReceiverType* receiver, TGenericGlobalListener<EventType>::EventCallback callback) { TGenericGlobalListener<EventType>::ConnectImpl(receiver, callback); }
...
template <class T> inline void Toshi::TGenericGlobalListener<T>::DisconnectImpl()
{
