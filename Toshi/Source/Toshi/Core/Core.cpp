...
const TCHAR* g_szAssertFilename;
size_t       g_uiAssertLineNumber;
const TCHAR* g_szAssertExpression;
...
INT_PTR CALLBACK AssertionDlgProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	switch (Message)
	{
		case WM_INITDIALOG:
		{
			...
			if (!g_szAssertFilename) g_szAssertFilename = "***INVALID***";

			if (!g_szAssertExpression) g_szAssertExpression = "***INVALID***";
...
AssertionAction TDebug_AssertionCallback(const TCHAR* file, TINT line, const TCHAR* expression)
{
	...
	if (hWnd != NULL) hWnd = GetLastActivePopup(hWnd);

	INT_PTR result = DialogBoxParamA(GetModuleHandleA(NULL), MAKEINTRESOURCEA(IDD_ASSERT), hWnd, AssertionDlgProc, NULL);

	return result != -1 ? AssertionAction(result) : AssertionAction::Break;
}
