...
template <class T, TINT MaxItems> class TFifo : public TGenericFifo
{
public:
	TFifo() { TGenericFifo::Create(m_Buffer, MaxItems, sizeof(T)); }

	~TFifo() { TGenericFifo::Destroy(); }

	TFifo(const TFifo&) = delete;
	TFifo(TFifo&&)      = delete;

	TBOOL Push(T* element, Flags flags = TGenericFifo::Flags_None) { return TGenericFifo::Push(element, flags); }

	TBOOL Pop(T* out, Flags flags = TGenericFifo::Flags_None) { return TGenericFifo::Pop(out, flags); }
