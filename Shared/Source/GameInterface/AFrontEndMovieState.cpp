...
void AFrontEndMovieState::OnInsertion()
{
    ...
    m_InputHelper.AddMapping(AInputMap::INPUTBUTTON_SPACE, TFALSE, -1.0f); // SPACE
    m_InputHelper.AddMapping(AInputMap::INPUTBUTTON_ENTER, TFALSE, -1.0f); // RETURN

    m_Background.Create(s_Assets[m_iAssetId]);
    m_Background.SetVisible(TTRUE);

    TIMPLEMENT_D("TXUI stuff");
