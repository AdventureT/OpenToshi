...
TBOOL ADX11MoviePlayer::PlayMovie(const char* fileName, uint32_t soundChannel, PlayFlags flags)
{
	...
	if (m_pFile != TNULL)
	{
		...
		if (m_TheoraDecoder != TNULL)
		{
			...
			m_FrameMS = TUINT((video->fps == 0.0) ? 0 : 1000.0 / video->fps);
	...
	if (IsMoviePlaying())
	{
		if (!IsMoviePaused())
		{
			...
			if (video != TNULL)
			{
				...
				TUINT8* src = video->pixels + pixelCount;
		...
		while (position > video->playms && m_FrameMS < position - video->playms)
		{
			...
			if (!video)
			{
				...
				if (m_bIsMovieLooping)
				{
					...
					TUINT8 flags = m_bIsMuted ? PlayFlags_Muted : 0;
